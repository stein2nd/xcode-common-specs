...
* この付録は、Xcode v26における「AI 伴走型開発」(例: ChatGPT in Xcode、Claude in Xcode) を **SPEC 準拠** で進めるための、運用ルール群を定めるものです。
* 本ガイドラインは、必須ではありません。「SPEC を唯一の正典としつつ AI を活用する開発スタイル」を推進するための「補助ルール」と考えてください。
...
* **ブランチ戦略**
  ...
  * CI/CD ワークフローの詳細については、`COMMON_SPEC_CICD.md` を参照
